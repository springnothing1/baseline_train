...
class LLaMA_adapter(nn.Module):
    ...
    def generate(
        ...
        tokens = torch.full((bsz, total_len), self.tokenizer.pad_id).to(imgs.device).long()

        for k, t in enumerate(prompts):
            tokens[k, : len(t)] = torch.tensor(t).to(imgs.device).long()
...
def load(name, llama_dir, llama_type="7B", device=None, download_root='ckpts', max_seq_len=512,
        phase="finetune"):
    if name in _MODELS:
