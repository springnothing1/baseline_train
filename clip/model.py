...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND, nn.Multiheadattention.forward的输入要求
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.vocab_size = vocab_size   # should be 77
        self.token_embedding = nn.Embedding(vocab_size, transformer_width)
        # learned position embedding
        self.positional_embedding = nn.Parameter(torch.empty(self.context_length, transformer_width))
        self.ln_final = LayerNorm(transformer_width)

        self.text_projection = nn.Parameter(torch.empty(transformer_width, embed_dim)) #(768, 512)
...
def build_model(state_dict: dict):
    ...
    if vit:
        ...
        # Determine whether it belongs to the attention module,determine the number of vit blocks
