...
class MSLSCLIP(Dataset):
    def __init__(self, root_dir, cities = '', nNeg = 5, transform = None, mode = 'train', task = 'im2im', subtask = 'all', seq_length = 1, posDistThr = 10, negDistThr = 25, cached_queries = 1000, cached_negatives = 1000, positive_sampling = True):
        ...
        for city in self.cities:
            ...
            # read the text description for each city
            with open(join(root_dir, subdir, city, 'query', 'descriptions.txt'), 'r') as qTextData:
                self.qText.extend(qTextData.readlines())
            with open(join(root_dir, subdir, city, 'database', 'descriptions.txt'), 'r') as dbTextData:
                self.dbText.extend(dbTextData.readlines())
        ...
        self.qText = np.asarray(self.qText)
        self.dbText = np.asarray(self.dbText)
    ...
    def __getitem__(self, idx):
        ...
        # load image text for all images in the triplet
