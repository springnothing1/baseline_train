...
def llama_create_save(data_iter, city, transform, model, path, prompt_load, device):
    ...
    for i, images in enumerate(data_iter):
        ...
        count += imgs.shape[0]
        if count % 100 == 0:
            print(f"===[{i}/{len_iter}]===create {count} images description already!!")
            
    with open(path, 'w') as f:
        for line in results:
            f.write(line + '\n')
    print(f"========create {count} images description for {city} query totally!!")
...
def main():
    ...
    for city in cities:
        ...
        query_path = os.path.join(root_dir, 'train_val', city, "query", "descriptions.csv")
        database_path = os.path.join(root_dir, 'train_val', city, "database", "descriptions.csv")
        # create text description and save 
        llama_create_save(q_data_iter, city, transform, model, query_path, prompt_load, device)
        llama_create_save(db_data_iter, city, transform, model, database_path, prompt_load, device)
